...
let declare_new_type t =
  let _ = ComAssumption.declare_variable NoCoercion ~kind:Decls.Definitional Constr.mkSet empty_named_universes_entry [] Glob_term.Explicit (CAst.make t) in
  Constr.mkVar t

let declare_new_variable v constr_t =
  let _ = ComAssumption.declare_variable NoCoercion ~kind:Decls.Definitional constr_t empty_named_universes_entry [] Glob_term.Explicit (CAst.make v) in
  Constr.mkVar v
